  ...
  if (plugins.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        <div className="w-12 h-12 rounded-xl bg-input-bg border border-border flex items-center justify-center mb-4">
        ...
        <p className="text-[11px] text-text-ghost mt-1">
          Add WASM plugins to <code className="font-mono text-[10px] bg-card-bg-hover px-1.5 py-0.5 rounded border border-border">~/.koompi-orch/plugins/</code>
        </p>
      </div>
    );
  }
      ...
      {plugins.map((plugin) => (
        ...
        >
          ...
          <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-[13px] font-bold font-mono shrink-0 ${
            plugin.enabled ? "bg-accent-muted text-accent" : "bg-card-bg-hover text-text-ghost"
          }`}>
              ...
              {plugin.capabilities.map((cap) => (
                <span
                  ...
                  className={`text-[9px] font-mono uppercase tracking-wider px-1.5 py-0.5 rounded ${
                    CAPABILITY_COLORS[cap] ?? "bg-card-bg-hover text-text-ghost"
                  }`}
                >
                  {cap}
                </span>
          ...
          <button
            ...
            className={`relative w-9 h-5 rounded-full transition-colors duration-200 flex-shrink-0 ${
              plugin.enabled ? "bg-accent" : "bg-elevated"
            }`}
          >
            <span
